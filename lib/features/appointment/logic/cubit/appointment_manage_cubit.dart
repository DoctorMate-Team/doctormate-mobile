...
class AppointmentManageCubit extends Cubit<AppointmentManageState> {
  AppointmentManageCubit(this._appointmentManageRepos)
    : super(AppointmentManageState.initial());
  ...
  Future<void> getPatientAppointments({
    ...
    final result = await _appointmentManageRepos.getPatientAppointments(
      page: page,
      limit: limit,
      status: status,
    );
    result.when(
      ...
      failure: (error) {
        emit(
          AppointmentManageState.errorAppointmentsPatient(
  ...
  Future<void> updateAppointmentStatus({required String appointmentId}) async {
    ...
    final result = await _appointmentManageRepos.updateAppointmentStatus(
    ...
    result.when(
      ...
      failure: (error) {
        emit(
          AppointmentManageState.errorUpdatingAppointmentStatus(
            message: error.apiErrorModel.message!,
          ),
        );
      },
  ...
  Future<void> rescheduleAppointment({
    ...
    emit(AppointmentManageState.reschedulingAppointment());
    final result = await _appointmentManageRepos.rescheduleAppointment(
      appointmentId: appointmentId,
      newDate: newDate,
      newTime: newTime,
      reason: reason,
    ...
    result.when(
      success: (response) {
        emit(AppointmentManageState.rescheduledAppointment(response.data));
      },
      failure: (error) {
        emit(
          AppointmentManageState.errorReschedulingAppointment(
            message: error.apiErrorModel.message!,
          ),
        );
      },
